// NextAuth 4 config for the vendor app. Credentials (email + password) →
// JWT session. The JWT embeds `organizationId` + `role` from the user's
// OrgMembership (a vendor user has a membership with role "vendor");
// `lib/session.ts` decodes that token for tenant scoping.
//
// Per-app cookie name (`vendor.session-token`) so the six apps don't clobber
