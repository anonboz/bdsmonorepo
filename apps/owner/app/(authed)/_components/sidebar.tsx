...
interface NavItem {
  href: string;
  /** Key under the `owner.nav` namespace. */
  key: string;
  icon: LucideIcon;
}
...
const NAV: NavItem[] = [
  ...
  { href: '/houses', key: 'houses', icon: Building2 },
  { href: '/tickets', key: 'tickets', icon: Wrench },
...
/**
 * Persistent owner navigation. Mobile-first per apps/owner/CLAUDE.md: a slide-in
 * drawer with a top bar by default, promoted to a fixed left rail on >=1024px.
 * Wraps all `(authed)` routes via the layout. Labels come from `@repo/i18n`.
 */
