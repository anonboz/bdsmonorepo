...
/** GET /api/work-orders?status=scheduled&take=20&skip=0 — this org's work orders. */
export async function GET(req: Request) {
  try {
    const session = await getSession();
    const url = new URL(req.url);
    const data = await listWorkOrders(session, {
    ...
    return handleRouteError(err, "Failed to list work orders");
  }
}
