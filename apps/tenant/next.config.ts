...
// Phase 11.1 — next-intl request-config loader lives at app root.
...
export default withSentryConfig(withSerwist(withNextIntl(config)), {
  ...
  project: 'bds-tenant-web',
  silent: !process.env.CI,
  authToken: process.env.SENTRY_AUTH_TOKEN,
  // `sourcemaps.disable` keeps the public bundle free of `.map`
  // siblings even though the build still uploads them to Sentry.
