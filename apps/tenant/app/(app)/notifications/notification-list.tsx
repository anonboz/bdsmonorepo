...
// Inbox rows + "mark all read". Client component because marking read is a
// mutation: PATCH/POST the API, drop the bell's cached count, and re-render the
// server-fetched list via router.refresh(). Rows arrive with ISO date strings
// (serializable across the RSC boundary) and are formatted here.
...
import { isNotificationType } from "@repo/shared";
          ...
          <CardContent className="p-0">
            <ul className="divide-y">
              {rows.map((n) => {
                ...
                const typeLabel = isNotificationType(n.type) ? t(`types.${n.type}`) : n.type;
                const inner = (
                  <>
                    <span
                      aria-hidden="true"
