...
// Create + manage this org's tenant announcements. The form POSTs to the API;
// the list toggles publish state (PATCH) and deletes (DELETE). All writes are
// re-validated + org-scoped server-side; we just refresh on success.
        ...
        <form onSubmit={onSubmit} className="space-y-3">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="Water shutoff Tuesday 9–11am"
            />
          </div>
          <div className="space-y-1">
...
function RowItem({ row }: { row: Row }) {
  ...
  async function remove() {
    setPending(true);
    await fetch(`/api/announcements/${row.id}`, { method: "DELETE" });
    setPending(false);
    router.refresh();
  }
