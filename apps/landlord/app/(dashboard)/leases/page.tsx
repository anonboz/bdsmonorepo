...
export default async function LeasesPage() {
  ...
  const { rows, total } = await listLeases(session, {});

  return (
    <div className="mx-auto max-w-5xl space-y-6 px-6 py-10">
      <header className="space-y-1">
        <h1 className="text-3xl font-semibold">Leases</h1>
        <p className="text-muted-foreground">{total} lease(s) in this organization.</p>
      </header>

      {rows.length === 0 ? (
        <Card>
          <CardContent className="py-12 text-center text-muted-foreground">
            No leases yet. Create one from a unit to get started.
          </CardContent>
        </Card>
      ) : (
        <Card>
